...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const success = await login(email, name.trim());
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      const success = await login("usuario@gmail.com", "Usuario Google");
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="items-center text-center">
          ...
          <Link href="/" className="flex items-center gap-2 mb-4">
            <div className="flex items-center justify-center w-14 h-14 bg-black dark:bg-white rounded-xl">
              <Check className="w-7 h-7 text-white dark:text-black" />
            </div>
          </Link>
          {/* Title & Subtitle */}
        ...
        <CardContent>
          ...
          {/* Google Sign Up Button - Same as mobile app */}
          <Button
            variant="outline"
          ...
          {/* Skip / Continue without account - Same as mobile app */}
          <button
            type="button"
            onClick={handleSkip}
